...
_ALL_PROTOCOLS = [
    # Paper protocols (UPEC, PEOC, and PEAC)
    # ======================================
    ExperimentProtocol(  # Study calibration for different SNR parameters
        ...
        measurement_snr_list=(
            ...
            ] + [  # Negative SNR rounded 3
                round(from_db(snr_db), 3)
                for snr_db in range(-30, 0, 3)
            ] + [  # Negative SNR rounded 5
                round(from_db(snr_db), 5)
                for snr_db in range(-42, -30, 3)
            ]
        ),
    ...
    # ===============
    ExperimentProtocol(
        protocol_name="20db_snr_uniform_phase",
        measurement_snr_list=[100.0],  # 20 dB
        measurement_von_mises_std_deg_list=[_MAX_STD_VON_MISES_DEG],  # Uniform phase error
        run_calibration_types=[
            CalibrationType.PEAC,
            CalibrationType.PEOC,
            # CalibrationType.UPEC,
            CalibrationType.UPEC_PATHS_PROJ,
            # CalibrationType.PEAC_FIXED_PRIOR
        ],
    ),
    ExperimentProtocol(
        protocol_name="std_vs_n_measurements",
        measurement_snr_list=[100.0],  # 20 dB
        measurement_von_mises_std_deg_list=[
            0.0,
            34.66,
