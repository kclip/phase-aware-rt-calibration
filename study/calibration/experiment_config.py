...
from src.data_classes import MeasurementNoiseType
from src.scenarios import ScenarioTheStrandMetadata, ScenarioToyExampleMetadata
...
EXPERIMENT_VERSION = os.getenv("STUDY_EXPERIMENT_VERSION", "v1")
EXPERIMENT_NAME = f"calibration_{EXPERIMENT_VERSION}"

_EXPERIMENT_CONFIG = get_experiment_config_from_version(EXPERIMENT_VERSION)
...
RX_TX_INDEXES = [(i, 0) for i in range(N_RX)]
...
CM_CARRIER_MODULATION = True

# Print progress
PRINT_VM_PARAMS = False
PRINT_FREQ = 1 if "light" in EXPERIMENT_VERSION else (PEAC_N_ITER_E_STEP + PEAC_N_ITER_M_STEP) * 5
...
coverage_map_metadata = CoverageMapMetadata(
    ...
    carrier_modulation=CM_CARRIER_MODULATION,
...
def _get_peac_calibration_metadata_base(
    normalization_constant: float,
    normalized_measurement_noise_std: float,
