...
from study.calibration.experiment_config import EXPERIMENT_VERSION
...
EXPERIMENT_FOLDER = os.path.join(PROJECT_FOLDER, "study", "calibration")
EXPERIMENT_PLOTS_FOLDER = os.path.join(EXPERIMENT_FOLDER, "saved_plots")


def protocol_plots_folder(protocol_name, n_loaded_runs: int):
    return os.path.join(
        EXPERIMENT_PLOTS_FOLDER,
        f"{EXPERIMENT_VERSION}_{protocol_name}_protocol_{n_loaded_runs}_loaded_runs"
    )
