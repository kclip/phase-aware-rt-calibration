...
) -> tf.Tensor:  # Shape [N_SUBCARRIERS]
    """
    Generate tensor of subcarrier frequencies; after carrier modulation if carrier_modulation=True, or before
    carrier modulation (baseband frequencies) if carrier_modulation=False.
    """
    baseband_frequencies = subcarrier_frequencies(
        num_subcarriers=n_subcarriers,
        subcarrier_spacing=subcarrier_spacing
    )
    if carrier_modulation:
        # Note: from Sionna v0.15.1, the baseband amplitudes (a^b_i) are returned instead of the passband amplitudes
        # (a_i). We have a^b_i = a_i e^{-j 2 pi f_c tau_i}, where tau_i is the propagation delay and f_c is the carrier
        # frequency. The frequencies below (f_b - f_c) are to be applied as :
        # a^b_i * e^{-j 2 pi (f_b - f_c) tau_i} = a_i e^{-j 2 pi f_b tau_i}
        return baseband_frequencies - scene.frequency
    else:
        return baseband_frequencies
