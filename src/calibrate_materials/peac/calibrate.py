...
from src.calibrate_materials.peac.free_energy import free_energy_von_mises
from src.calibrate_materials.peac.von_mises_parameters import compute_von_mises_posterior_mean, \
    compute_von_mises_posterior_concentration, compute_von_mises_global_concentration
...
def phase_error_aware_calibration(
    ...
    bypass_grads_e_step = (  # Avoid useless computation of the free-energy when using only closed-form updates
    ...
    for step in range(calibration_metadata.n_steps):
        ...
        if not calibration_metadata.use_gradient_von_mises_mean:
            vm_mean_variables.assign(
                compute_von_mises_posterior_mean(
                    ...
                    measurement_noise_std=calibration_metadata.measurement_noise_std
                )
            )
        ...
        for e_iter in range(calibration_metadata.n_iter_e_step):
            ...
            if bypass_grads_e_step:
                loss_iter_e_step = loss_iter_m_step  # Placeholder: display/log same loss as M-step
            else:  # Gradient-based updates
                # Forward pass
            ...
            if print_von_mises_params:
                display[0] += (
                    ...
                    f"  - Von Mises means: {trackers['von_mises_means'][n_e_step]}\n" +
                    f"  - Von Mises concentration: {trackers['von_mises_concentrations'][n_e_step]}"
                )
