...
def free_energy_von_mises(
    ...
    von_mises_amortize_concentration: bool,  # If True, share the same variational concentration across all paths
    ...
    # Measurement noise
    meas_noise_power = measurement_noise_std ** 2
    ...
    cfr_moment_1 = tf.squeeze(  # [N_MEASUREMENTS, N_RX_TX_PAIRS, N_SUBCARRIERS*N_ARR_RX*N_ARR_TX]
        (
            cfr_flat @  # [N_RX_TX_PAIRS, N_SUBCARRIERS*N_ARR_RX*N_ARR_TX, N_PATHS]
    ...
    squared_cfr_var = (  # [N_MEASUREMENTS, N_RX_TX_PAIRS, N_PATHS]
        squared_cfr *  # [N_RX_TX_PAIRS, N_PATHS]
    ...
    expected_nll_log_exp = inv_meas_noise_power * (
        squared_norm(cfr_moment_1 - measurements_flat, axis=-1) +
        tf.reduce_sum(squared_cfr_var, axis=2)
    )
    expected_nll = expected_nll_log_partition + expected_nll_log_exp
