...
from src.data_classes import MeasurementNoiseType, MeasurementDataPhaseNoise, MeasurementDataPositionNoise
...
def compute_selected_cfr_from_paths_traces(
    ...
    cfr_per_mpc = compute_cfr_per_mpc_from_paths(
        ...
        carrier_modulation=measurement_metadata.carrier_modulation,
...
def simulate_measurements(
    scene: Scene,
    measurement_metadata: _MeasurementMetadataBase
) -> Union[MeasurementDataPhaseNoise, MeasurementDataPositionNoise]:
    ...
    normalization_constant = float(np.sqrt(mean_channel_power)) if measurement_metadata.normalize else 1.0
    ...
    if measurement_metadata.with_additive_noise:
        ...
        normalized_measurement_noise_std = None
